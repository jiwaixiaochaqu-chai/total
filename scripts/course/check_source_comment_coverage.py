...
第三方压缩库、生成站点与扩展模块目录不属于 V1 注释维护边界。
...
class CommentCoverageIssue:
    ...
    字段：
        path: 相对项目根目录的文件路径。
        line: 问题所在行号。
        kind: 问题类型（module_docstring / public_docstring / file_header / parse_error）。
        symbol: 涉及的符号名；模块或文件层面为空字符串。
        message: 人类可读的问题描述。

    调用顺序：check_python_file() / check_javascript_files() / check_powershell_files() -> CommentCoverageIssue(...)。
    """
...
def _is_v1_path(path: Path) -> bool:
    ...
    GraphRAG 和运维扩展目录不属于 V1 注释维护边界。

    参数：
        path: 待判断的文件绝对路径。

    返回：
        True 表示属于 V1 自有代码；False 表示被排除。

    调用顺序：iter_python_files() / check_javascript_files() -> _is_v1_path()。
    """

    relative = path.relative_to(PROJECT_ROOT)
    if any(part.lower() in EXTENSION_PATH_PARTS for part in relative.parts):
        return False
    return True
...
def iter_python_files() -> Iterable[Path]:
    ...
    参数：
        无（扫描范围由模块级 PYTHON_ROOTS 常量决定）。

    返回：
        去重后的 V1 Python 文件路径生成器。

    调用顺序：build_report() -> iter_python_files() -> _is_v1_path()。
    """
...
def _public_nodes(tree: ast.Module) -> Iterable[ast.AST]:
    ...
    只遍历模块顶层节点；类定义只展开其直接子节点中的公开方法。

    参数：
        tree: 已解析的 ast.Module 节点。

    返回：
        公开节点生成器（FunctionDef / AsyncFunctionDef / ClassDef）。

    调用顺序：check_python_file() -> _public_nodes()。
    """
...
def check_python_file(path: Path) -> list[CommentCoverageIssue]:
    ...
    解析失败时返回一条 parse_error 问题，不阻断其它文件检查。

    参数：
        path: 待检查的 Python 文件路径。

    返回：
        CommentCoverageIssue 列表；没有问题时为空列表。

    调用顺序：build_report() -> check_python_file() -> _public_nodes()。
    """
...
def check_javascript_files() -> list[CommentCoverageIssue]:
    ...
    参数：
        无（扫描目录由模块级 JAVASCRIPT_ROOT 常量决定）。

    返回：
        缺少文件头注释的问题列表。

    调用顺序：build_report() -> check_javascript_files() -> _is_v1_path()。
    """
...
def check_powershell_files() -> list[CommentCoverageIssue]:
    ...
    参数：
        无（扫描目录由模块级 POWERSHELL_ROOT 常量决定）。

    返回：
        缺少 <# #> 帮助头的问题列表。

    调用顺序：build_report() -> check_powershell_files()。
    """
...
def build_report() -> dict[str, object]:
    """汇总注释覆盖结果，供本地检查和发布门禁消费。（★★★ 核心）

    依次检查 Python / JavaScript / PowerShell，汇总成统一报告；无任何问题时 ok 为 True。

    参数：
        无。

    返回：
        覆盖报告字典（checked_python_files / issue_count / issues / ok）。

    调用顺序：main() -> build_report() -> iter_python_files() / check_python_file() /
        check_javascript_files() / check_powershell_files()。
    """
...
def parse_args() -> argparse.Namespace:
    ...
    返回：
        解析后的命名空间（output / fail_on_issues）。
...
def main() -> None:
    ...
    返回：
        无；--fail-on-issues 且存在问题时以退出码 1 结束。

    调用顺序：命令行入口 -> main() -> parse_args() -> build_report()。
    """
