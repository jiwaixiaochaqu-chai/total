...
TREE = {
    ...
    "note": "19 讲 · 4 阶段 · 8 附录 · 17 个动画（15 个章节动画 + 2 个综合动画）\nLangChain + Milvus + FastAPI",
    ...
    "children": [
        ...
        # ═══ Phase 2 ═══
        {
            ...
            "note": "第 03–10 讲 · LangChain → Milvus → 意图 → 检索 → 重写 → 混合搜索 → 编排 → 流水线与提示词",
            ...
            "children": [
                ...
                {
                    "id": "L10", "title": "第 10 讲 · RAG Pipeline 主流程与 Prompt 生成",
                    "note":"8 阶段事件驱动模型；FAQ 快速路径 vs 标准问答；上下文、Prompt Profile、引用增强与流式收口",
                    "color":PALETTE["phase2"],
                    "children": [
                        ...
                        {"id":"L10d","title":"Prompt Profile","note":"build_answer_prompt_profile；按风险类别、意图和默认优先级选择回答口径","color":PALETTE["phase2"]},
                        {"id":"L10e","title":"场景变量注入","note":"assistant_name、business_domain、support_contact 注入回答模板","color":PALETTE["phase2"]},
                        {"id":"L10f","title":"Prompt 组装","note":"prepare_answer；把 history、question、context 填入 system_prompt 与 user_prompt","color":PALETTE["phase2"]},
                        {"id":"L10g","title":"引用增强","note":"答案引用标注；来源文档溯源","color":PALETTE["phase2"]},
                        {"id":"L10h","title":"流式输出终止","note":"_finish_with_single_answer；SSE 流结束信号","color":PALETTE["phase2"]},
                    ]
                },
            ]
        },
        # ═══ Phase 3 ═══
        {
            ...
            "note": "第 11–12 讲 · FastAPI 异步框架 · 应用入口 · 启动前校验",
            "color": PALETTE["phase3"],
            "children": [
                {
                    "id": "L11", "title": "第 11 讲 · FastAPI 与异步 Web",
                    "note":"同步 vs 异步编程；FastAPI 路由、中间件、依赖注入；WebSocket 流式输出",
                    "color":PALETTE["phase3"],
                    "children": [
                        {"id":"L11a","title":"同步 vs 异步","note":"async/await 原理；Web 服务中的并发模型","color":PALETTE["phase3"]},
                        {"id":"L11b","title":"FastAPI 核心","note":"路由 · 中间件 · 依赖注入 (Depends)","color":PALETTE["phase3"]},
                        {"id":"L11c","title":"WebSocket 端点","note":"流式问答推送；SSE 事件格式","color":PALETTE["phase3"]},
                        {"id":"L11d","title":"API 路由层","note":"chat.py · admin.py · pages.py；请求解析与错误处理","color":PALETTE["phase3"]},
                        {"id":"L11e","title":"用户反馈","note":"FeedbackStore.add_feedback；点赞/点踩收集","color":PALETTE["phase3"]},
                    ]
                },
                {
                    "id": "L12", "title": "第 12 讲 · 应用入口与环境校验",
                    "note":"Preflight Check 设计模式；启动时依赖完整性验证；app.py 逐行解读",
                    "color":PALETTE["phase3"],
                    "children": [
                        {"id":"L12a","title":"Preflight 设计","note":"启动即验证；依赖不完整则拒绝启动","color":PALETTE["phase3"]},
                        {"id":"L12b","title":"app.py 逐行解读","note":"create_app 完整流程；从配置加载到路由注册","color":PALETTE["phase3"]},
                        {"id":"L12c","title":"值校验","note":"_is_placeholder；占位符检测 · 配置有效性","color":PALETTE["phase3"]},
                        {"id":"L12d","title":"路径校验","note":"_require_path；模型路径 · 数据目录","color":PALETTE["phase3"]},
                        {"id":"L12e","title":"运行时环境验证","note":"validate_runtime_environment；全量依赖完整性检查","color":PALETTE["phase3"]},
                    ]
                },
            ]
        },
        # ═══ Phase 4 ═══
        {
            "id": "p4", "title": "第四阶段：治理与生产化（5 个学习单元）",
            "note": "讲义主线 13–17；源码实现映射保留原 13–19 编号",
            "color": PALETTE["phase4"],
            "children": [
                {
                    "id": "L13", "title": "第 13 讲 · 知识库治理与离线入库发布",
                    "note":"把版本、DataScope、入库、质量门禁和 active 切换串成一次发布任务；映射源码 13–15 及原 16 的入库质量部分",
                    "color":PALETTE["phase4"],
                    "children": [
                        {"id":"L13a","title":"数据边界","note":"DataScope：tenant · dataset · visibility · roles","color":PALETTE["phase4"]},
                        {"id":"L13b","title":"候选版本","note":"STAGED → ACTIVE → ARCHIVED；active 指针和回滚","color":PALETTE["phase4"]},
                        {"id":"L13c","title":"入库主链路","note":"Loader → normalize → split → embedding → Milvus","color":PALETTE["phase4"]},
                        {"id":"L13d","title":"增量决策","note":"IndexManifest；跳过、复用、重建和失效","color":PALETTE["phase4"]},
                        {"id":"L13e","title":"质量发布","note":"质量报告 → Gate → 激活；失败时保留候选版本","color":PALETTE["phase4"]},
                    ]
                },
                {
                    "id": "L14", "title": "第 14 讲 · RAG 质量评测与 Bad Case 闭环",
                    "note":"评估检索和答案是否可靠；映射原 16 的 RAG Evaluation 部分",
                    "color":PALETTE["phase4"],
                    "children": [
                        {"id":"L14a","title":"评测指标","note":"Recall@K · MRR · 关键词覆盖 · 场景隔离","color":PALETTE["phase4"]},
                        {"id":"L14b","title":"答案校验","note":"引用有效性 · 覆盖率 · 上下文匹配","color":PALETTE["phase4"]},
                        {"id":"L14c","title":"Bad Case","note":"报告筛选 → 人工复核 → eval_sets 回归样本","color":PALETTE["phase4"]},
                        {"id":"L14d","title":"RAG Gate","note":"Evaluation 结果作为质量证据，阻断退化发布","color":PALETTE["phase4"]},
                    ]
                },
                {
                    "id": "L15", "title": "第 15 讲 · 自动化测试与接口验收",
                    "note":"回答如何证明代码、接口和已有功能没有被改坏；映射原 17",
                    "color":PALETTE["phase4"],
                    "children": [
                        {"id":"L15a","title":"测试分层","note":"纯逻辑 → API 保护 → Smoke → 回归","color":PALETTE["phase4"]},
                        {"id":"L15b","title":"核心函数测试","note":"绕过 HTTP，直接验证意图、检索、Prompt 和验收逻辑","color":PALETTE["phase4"]},
                        {"id":"L15c","title":"接口合同","note":"状态码、事件顺序、字段结构和错误响应","color":PALETTE["phase4"]},
                        {"id":"L15d","title":"自动验收","note":"guardrails、Smoke Test、非零退出码和 CI","color":PALETTE["phase4"]},
                    ]
                },
                {
                    "id": "L16", "title": "第 16 讲 · 线上可观测性、Trace 与容量治理",
                    "note":"线上发现和定位问题，评估高并发扩展；映射原 18",
                    "color":PALETTE["phase4"],
                    "children": [
                        {"id":"L16a","title":"Trace 结构","note":"RAGQueryContext · metadata · 阶段耗时 · 首 token","color":PALETTE["phase4"]},
                        {"id":"L16b","title":"线上归因","note":"质量、性能、部署和依赖问题分流","color":PALETTE["phase4"]},
                        {"id":"L16c","title":"压测容量","note":"并发、延迟、连接池、模型服务和扩容判断","color":PALETTE["phase4"]},
                        {"id":"L16d","title":"扩展架构","note":"网关限流、API 多副本、模型服务池、Milvus 集群","color":PALETTE["phase4"]},
                    ]
                },
                {
                    "id": "L17", "title": "第 17 讲 · Docker 交付与排障",
                    "note":"完成镜像、Compose、初始化、验收和排障；映射原 19",
                    "color":PALETTE["phase4"],
                    "children": [
                        {"id":"L17a","title":"镜像配置","note":"Dockerfile、基础镜像、模型挂载和运行时变量","color":PALETTE["phase4"]},
                        {"id":"L17b","title":"Compose 编排","note":"MySQL · Redis · Milvus · etcd · MinIO · API","color":PALETTE["phase4"]},
                        {"id":"L17c","title":"部署验收","note":"构建、启动、初始化知识库、API 和依赖检查","color":PALETTE["phase4"]},
                        {"id":"L17d","title":"故障排查","note":"端口、挂载、环境变量、健康检查和日志定位","color":PALETTE["phase4"]},
                    ]
                },
            ]
        },
        ...
        # ═══ 动画 ═══
        {
            "id": "animations", "title": "业务流程图动画 (17 个)",
            "note": "每讲配套动画 · 综合流水线演示 · Mermaid + HTML 动画",
            "color": PALETTE["anim"],
            "children": [
                {"id":"ani1","title":"章节动画 × 15","note":"第 05-19 讲功能闭环地图；节点与代码对齐动画","color":PALETTE["anim"]},
