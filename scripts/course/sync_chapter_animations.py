...
#
# CHAPTERS 是核心数据源，包含 05~10、11~19 章每章的文件路径、符号定义
...
CHAPTERS: list[dict[str, object]] = [
    ...
    },
    {
        ...
        "nodes": [
            ...
            ("OUT", "章节输出", "qa_core/application/service.py", "QAService.stream_query() / debug_retrieval()", "为第 10 章 Pipeline 下沉和第 11 章 API 入口提供稳定服务接口。"),
        ],
    },
    {
        "no": "10",
        ...
        "desc": "把 QAService 内部编排下沉为可观察的 Stage 0-7 Pipeline，并在生成阶段完成 Prompt Profile 选择与回答提示词组装。",
        "nodes": [
            ...
            ("PREP", "检索准备", "qa_core/pipeline/steps.py", "prepare_retrieval()", "生成 RetrievalPreparation，并在进入检索前选择 PromptProfile。"),
            ("PROFILE", "Profile 结构", "qa_core/prompts/profiles.py", "PromptProfile", "承载 system_template、user_template 和 profile_name。"),
            ("SELECT", "Profile 选择", "qa_core/prompts/selector.py", "build_answer_prompt_profile()", "按风险类别、意图和默认优先级选择回答口径。"),
            ("SCENARIO", "场景变量", "qa_core/prompts/selector.py", "_scenario_prompt_context()", "把助手名称、业务域和支持联系方式注入模板。"),
            ...
            ("PROMPT", "Prompt 组装", "qa_core/pipeline/steps.py", "prepare_answer()", "把历史、问题、上下文填入选定 Profile，形成 system_prompt 和 user_prompt。"),
            ("ANSWER", "生成与结束", "qa_core/pipeline/steps.py, qa_core/pipeline/rag.py", "stream_llm_answer() / _finish_with_single_answer()", "流式生成、补强引用、保存历史并 finish_success。"),
        ],
    },
    {
        "no": "11",
        "title": "FastAPI 与异步 Web 框架",
        "doc": "11-fastapi-async.md",
        "html": "11-fastapi-service.html",
        "path": "codealong/chapters/ch11_fastapi_service",
    ...
    },
    {
        "no": "12",
        "title": "应用入口与环境前置校验",
        "doc": "12-app-entry-preflight.md",
        "html": "12-preflight-checks.html",
        "path": "codealong/chapters/ch12_preflight_checks",
    ...
    },
    {
        "no": "13",
        "title": "知识库多版本管理",
        "doc": "13-kb-versioning.md",
        "html": "13-kb-versioning.html",
        "path": "codealong/chapters/ch13_kb_versioning",
    ...
    },
    {
        "no": "14",
        "title": "数据隔离与多租户",
        "doc": "14-data-isolation.md",
        "html": "14-data-isolation.html",
        "path": "codealong/chapters/ch14_data_isolation",
        ...
        "nodes": [
            ...
            ("TEST", "隔离测试", "tests/test_data_scope.py", "DataScopeChapter14Test", "用测试锁住 DataScope 解析、过滤表达式和检索入参。"),
        ],
    },
    {
        "no": "15",
        "title": "文档入库与索引链路",
        "doc": "15-ingestion-pipeline.md",
        "html": "15-ingestion-pipeline.html",
        "path": "codealong/chapters/ch15_ingestion_pipeline",
        ...
        "nodes": [
            ...
            ("REUSE", "引用式复用", "qa_core/indexing/service.py", "_ingest_single_file()", "未变化 chunk 不复制，目标 manifest 直接引用；修改/删除时由 expire_documents_for_version() 写 valid_to_seq。"),
    ...
    },
    {
        "no": "16",
        "title": "RAG 回归验收与入库质量",
        "doc": "16-quality-evaluation.md",
        "html": "16-quality-evaluation.html",
        "path": "codealong/chapters/ch16_quality_evaluation",
        ...
        "nodes": [
            ...
            ("TEST", "回归测试", "tests/test_quality_report.py", "QualityReportChapter16Test", "锁住质量报告结构和关键告警。"),
        ],
    },
    {
        "no": "17",
        "title": "测试与接口验收",
        "doc": "17-testing-system.md",
        "html": "17-testing-system.html",
        "path": "codealong/chapters/ch17_test_system",
        "desc": "把章节结构、单元测试、QAService/API 冒烟、检索诊断和质量报告收束为自动化验收入口。",
        "nodes": [
            ("UNIT", "单元测试", "tests/test_test_system.py", "TestSystemChapter17Test", "覆盖章节测试系统本身。"),
    ...
    },
    {
        ...
        "doc": "18-observability-tracing.md",
        "html": "18-observability-tracing.html",
        "path": "codealong/chapters/ch18_observability_tracing",
    ...
    },
    {
        ...
        "doc": "19-docker-containerization.md",
        "html": "19-docker-containerization.html",
        "path": ".",
        "desc": "把已经验收和观测过的 RAG 服务交付到可重复启动、可检查和可排障的 Docker Compose 环境。",
        "nodes": [
            ("ENV", "部署配置", ".env.compose.example", "APP_BASE_IMAGE", "定义镜像、服务地址、模型挂载和运行时环境变量。"),
            ("BASE", "基础镜像", "Dockerfile.base", "FROM python:3.12-slim", "安装系统依赖和重型 Python 运行依赖，稳定构建底座。"),
            ("IMAGE", "应用镜像", "Dockerfile", "FROM ${APP_BASE_IMAGE}", "复制项目代码并生成 API 服务镜像。"),
            ("COMPOSE", "服务编排", "docker-compose.yml", "services", "编排 MySQL、Redis、Milvus、etcd、MinIO 和 API 的网络、卷和健康检查。"),
            ("DEPLOY", "部署脚本", "scripts/deploy/deploy_docker.ps1", "Invoke-Compose", "按顺序构建、启动依赖、初始化并检查服务。"),
            ("VERIFY", "新环境验收", "scripts/deploy/verify_fresh_docker_deploy.py", "main()", "验证 Compose 配置、容器状态、API 和依赖服务是否可用。"),
            ("OUT", "可交付运行环境", "docker-compose.yml", "services", "完整项目能以一致方式启动、验收并定位部署问题。"),
        ],
    },
]
...
BUSINESS_NODES_BY_CHAPTER: dict[str, list[tuple[str, str, str, str]]] = {
    ...
    "09": [
        ...
        ("B_OUT", "交付服务层 API", "本章交付", "为第 10 章 Pipeline 下沉和第 11 章 Web API 提供稳定入口。"),
    ],
    "10": [
        ...
        ("B_PREPARE", "准备检索参数", "Stage 2", "加载历史、分类、改写、计划、变体，并选择 PromptProfile。"),
        ("B_PROFILE", "选择回答口径", "生成准备", "按风险类别、意图和默认优先级选择模板，并注入场景变量。"),
        ("B_RETRIEVE", "FAQ 与文档召回", "证据获取", "先 FAQ 后文档，按计划检索候选。"),
        ("B_ANSWER", "组织证据并回答", "生成闭环", "构造上下文和 Prompt，生成回答或明确说明信息不足。"),
    ...
    "19": [
        ("B_CONFIG", "准备部署配置", "交付输入", "明确镜像、服务地址、模型目录和环境变量。"),
        ("B_BUILD", "构建基础与应用镜像", "镜像构建", "先缓存重型依赖，再复制业务代码，缩短日常交付构建时间。"),
        ("B_COMPOSE", "编排依赖服务", "运行拓扑", "让 API、MySQL、Redis、Milvus、etcd 和 MinIO 在同一网络协作。"),
        ("B_START", "启动并初始化", "部署执行", "按依赖顺序拉起服务，完成知识库和应用启动准备。"),
        ("B_VERIFY", "执行交付验收", "可验证交付", "检查 Compose 配置、容器健康和 API 可访问性。"),
        ("B_OUT", "定位部署问题", "本章交付", "通过日志、容器状态、挂载路径和服务健康检查完成排障闭环。"),
    ],
}
...
def main() -> None:
    """生成全部当前章节（05-10、11-19）的独立 HTML 动画页面。
