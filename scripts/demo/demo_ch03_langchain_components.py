...
使用方式：
    python scripts/demo/demo_ch03_langchain_components.py
"""
...
def demo_history_storage() -> dict[str, object]:
    """用临时 SQLite 演示历史消息的写入、重载、隔离和清空。

    生产项目把同一个 LangChain 适配器连接到 MySQL；这里使用临时 SQLite，
    让第 03 章 demo 无需启动数据库也能验证持久化行为。

    返回:
        包含各阶段消息数量的演示结果。

    调用顺序：命令行入口 -> demo_history_storage()。
    """
    with TemporaryDirectory(prefix="knowforge-history-") as temp_dir:
        database_path = Path(temp_dir) / "history.db"
        engine = create_engine(f"sqlite:///{database_path.as_posix()}")
        first = SQLChatMessageHistory(
        ...
        first.add_messages([
            ...
            AIMessage(content="通常包括材料提交、合同签署和账号开通。"),
        ])
        ...
        isolated = SQLChatMessageHistory(
            session_id="session-b",
            connection=engine,
            table_name="demo_chat_messages",
        )
        before_clear = len(reloaded.messages)
        isolated_count = len(isolated.messages)
        reloaded.clear()
        after_clear = len(reloaded.messages)
        engine.dispose()
        return {
            "database": "temporary SQLite file",
            "reloaded_messages": before_clear,
            "isolated_session_messages": isolated_count,
            "messages_after_clear": after_clear,
        }
...
def main() -> None:
    ...
    执行流程:
        ...
        3. SQLChatMessageHistory：展示历史消息如何持久化和按 session 隔离
        4. Document + metadata：展示文档和元数据如何组织
        5. RecursiveCharacterTextSplitter：展示中文文本切分效果
        6. 映射到项目代码位置：帮助读者找到实际生产代码
    """
    question = "入职流程需要哪些审批？"
    ...
    print("\n[3] SQLChatMessageHistory 历史持久化")
    pprint(demo_history_storage())
