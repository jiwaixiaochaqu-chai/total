...
#   - avg_answer_confidence_score：最终答案置信度均值（观察项，不替代召回/关键词指标）；
#   - 错误率和平均耗时。
#
...
from scripts.common import PROJECT_ROOT, configure_utf8_stdio

# EvalCaseRuntime: 评测样本运行时参数解析
# load_eval_items: 读取评测集 JSON
from scripts.eval_common import EvalCaseRuntime, load_eval_items, resolve_eval_dataset_path
...
def find_expected_source_rank(
    ...
    *,
    ...
    调用顺序：命令行入口 -> find_expected_source_rank()。
    """
...
def _prompt_profile_name(retrieval: dict[str, Any], debug_payload: dict[str, Any]) -> str:
    ...
    调用顺序：命令行入口 -> _prompt_profile_name()。
    """
...
def _effective_source_name(
    ...
    调用顺序：命令行入口 -> _effective_source_name()。
    """
...
def run_case(
    *,
    ...
    try:
        # ---------- 第一步：debug_retrieval —— 只看检索有没有命中 ----------
        # 把”召回有没有命中”与”最终生成有没有说对”分开诊断。
        # 如果 debug 已经没命中预期来源 → 问题在入库、query_variants、过滤或阈值；
        # 如果 debug 命中了但最终答案不对 → 问题在 prompt 和模型输出。
    ...
    try:
        # ---------- 第二步：stream_query —— 真实流式问答 ----------
        # 逐 event 收集 token、end、error 三类事件
        for event in service.stream_query(
            ...
            **runtime.service_kwargs(),
            ...
            if event["type"] == "token":
                # 流式生成片段，逐 token 拼接最终答案
                answer += event.get("token", "")
            elif event["type"] == "end":
                # 流结束事件，携带完整的检索结果和路由决策
                ...
                answer_confidence = event.get("answer_confidence") or retrieval.get("answer_confidence") or {}
    ...
    return {
        ...
        "answer_confidence_reasons": answer_confidence.get("reasons") or [],
...
def default_output_path(dataset: str) -> Path:
    ...
    name = resolve_eval_dataset_path(dataset).stem or "eval"
...
def evaluate_dataset(args: argparse.Namespace) -> dict[str, Any]:
    ...
    调用顺序：命令行入口 -> evaluate_dataset()。
    """
    data = load_eval_items(args.dataset, args.limit)
    service = get_qa_service()
    # 逐条执行评测样本（顺序执行，保证 session 和状态隔离）
    rows = [run_case(service=service, item=item, index=index, args=args) for index, item in enumerate(data, start=1)]

    # ---------- 按标注维度筛选评估子集 ----------
    # 每个子集只包含有对应标注字段的样本，未标注样本不参与该维度计算
    evaluated_source_cases = [row for row in rows if row["source_recall_hit"] is not None]         # 有预期来源标注
    hit_type_cases = [row for row in rows if row["expected_hit_type"]]                              # 有预期命中类型标注
    source_inference_cases = [row for row in rows if row["expected_effective_source"]]              # 有预期 source 标注
    prompt_profile_cases = [row for row in rows if row["expected_prompt_profile"]]                  # 有预期 Prompt Profile 标注
    confidence_cases = [row for row in rows if row["answer_confidence_score"] is not None]          # 有答案置信度
    hit_type_counts = Counter(row["hit_type"] or "unknown" for row in rows)
    answer_confidence_levels = Counter(row["answer_confidence_level"] or "unknown" for row in confidence_cases)
    ...
    scenario_metrics = {
        scenario: {
            ...
            "avg_answer_confidence_score": round(
                sum(float(item["answer_confidence_score"] or 0.0) for item in items if item["answer_confidence_score"] is not None)
                / max(sum(1 for item in items if item["answer_confidence_score"] is not None), 1),
                4,
            ),
    ...
    return {
        ...
        "dataset": str(resolve_eval_dataset_path(args.dataset)),
        "created_at": datetime.now(timezone.utc).isoformat(),
        "total": len(rows),
        "errors": sum(1 for row in rows if row["error"]),
        "avg_keyword_coverage": round(sum(row["keyword_coverage"] for row in rows) / max(len(rows), 1), 4),
        "avg_answer_confidence_score": (
            round(sum(float(row["answer_confidence_score"] or 0.0) for row in confidence_cases) / len(confidence_cases), 4)
            if confidence_cases
            else 0.0
        ),
        "recall_at_k": round(
        ...
        "answer_confidence_levels": dict(answer_confidence_levels),
