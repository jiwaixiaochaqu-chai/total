...
def candidate_values(start: float = 0.50, end: float = 0.95, step: float = 0.01) -> list[float]:
    ...
    参数：
        start: 候选阈值起点（含），默认 0.50。
        end: 候选阈值终点（含），默认 0.95。
        step: 扫描步长，默认 0.01。

    返回：
        从小到大排列的阈值候选列表（按 4 位小数取整）。
...
def evaluate_binary_threshold(observations: list[BinaryObservation], threshold: float) -> dict[str, Any]:
    ...
    参数：
        observations: 阈值样本列表（分数、标签、误判代价）。
        threshold: 待评估的直出分数阈值。

    返回：
        包含 tp/fp/tn/fn、precision、recall、false_direct_rate 和
        weighted_loss 的评估结果字典。
...
def select_faq_threshold(
    ...
    *,
    ...
    参数：
        observations: FAQ 直出阈值样本列表。
        min_precision: 业务要求的最低精确率。
        min_recall: 业务要求的最低召回率。
        max_false_direct_rate: 允许的最大误直出率。
        current_threshold: 当前生产阈值，用于可行解平局时优先选择更接近现状的候选。

    返回：
        包含 ok（是否存在满足约束的可行解）、selected（最优候选或 None）、
        constraints（约束快照）和 scan（全量阈值扫描结果）的字典。
...
def load_json_list(path: str | Path) -> list[dict[str, Any]]:
    ...
    参数：
        path: JSON 数据集文件路径（字符串或 Path 对象）。

    返回：
        数据集内的对象字典列表。

    异常：
        ValueError: 顶层不是 JSON list 时抛出。
...
def collect_faq_observations(cases: list[dict[str, Any]]) -> tuple[list[BinaryObservation], list[dict[str, str]]]:
    ...
    逐个用例调用 debug_retrieval 获取 FAQ 直出分数，任一用例执行失败时
    仅记录失败信息，不中断整体校准。

    参数：
        cases: FAQ 阈值校准用例列表（question / expected_faq_direct 等字段）。

    返回：
        (observations, failures) 二元组：成功提取的 BinaryObservation 列表，
        以及失败用例的 case_id + 错误信息列表。
...
def _top_candidate(candidates: list[dict[str, Any]], source: str) -> dict[str, Any] | None:
    """从候选意图中选出指定来源（rule/model）中分数最高的一条。

    参数：
        candidates: 候选意图列表（来自 row["intent"]["candidate_intents"]）。
        source: 候选来源标识，取 "rule" 或 "model"。

    返回：
        该来源中分数最高的候选字典；该来源没有任何候选时返回 None。

    调用顺序：_simulate_intent() -> _top_candidate()。
    """
...
def _simulate_intent(case: dict[str, Any], row: dict[str, Any], model_min_score: float) -> str:
    """离线复现生产意图仲裁，判断给定阈值下模型是否接管规则。

    与 qa_core/intent/decision.py 的 _fuse_decision 决策树保持一致，
    但只输出最终意图字符串（不模拟置信度分数和决策策略标签），
    用于在多个 model_min_score 候选上批量计算准确率。

    参数：
        case: 意图策略评测样本（含 expected_intent / history / critical 等字段）。
        row: evaluate_case() 产出的单条评测结果（含 intent.candidate_intents）。
        model_min_score: 当前模拟的模型最低接管分数阈值。

    返回：
        仲裁后的最终意图字符串。

    调用顺序：select_intent_model_threshold() -> _simulate_intent() -> _top_candidate()。
    """
    ...
    # 规则或模型任一缺失时无法仲裁，退化为直接取样本记录中的原始意图
    ...
    # 优先级1：模型判追问但无历史 → 模型误判，保持规则
    # 原因：BERT 容易把短句误判为追问，无历史上下文时追问没有意义
    if model_intent == "FOLLOW_UP" and not has_history:
        return rule_intent
    # 优先级2：模型置信度低于阈值 → 不信任模型，保持规则
    # 原因：model_min_score 基于离线评测确定，低于该值时模型准确率显著下降
    if model_score < model_min_score:
        return rule_intent
    # 优先级3：规则与模型一致 → 互相印证，直接采纳（模拟只关心意图，省略分数加成）
...
def select_intent_model_threshold(
    ...
    *,
    ...
    在 0.40 ~ 0.90 区间逐阈值模拟仲裁，按加权错误数 + 准确率选出最优阈值；
    加权错误数中 critical 用例按 5 倍计权，避免高风险样本被普通样本稀释。

    参数：
        cases: 意图策略评测样本列表。
        rows: evaluate_case() 对每个 case 产出的评测结果列表（与 cases 严格一一对应）。
        current_threshold: 当前生产阈值，用于平局时优先选择更接近现状的候选。

    返回：
        包含 ok（有效样本数是否 >= 6）、selected（最优阈值行）和
        scan（全量扫描结果）的字典。
...
def build_parser() -> argparse.ArgumentParser:
    ...
    返回：
        argparse.ArgumentParser 实例，参数默认值对齐 DEFAULT_* 常量。
...
def main() -> int:
    ...
    校准结果只落盘候选报告（applied=False），由人工复核后再修改生产配置，
    脚本本身不做任何自动变更。

    返回：
        进程退出码：0 表示成功；1 表示样本不足且启用了 --fail-on-insufficient。
    ...
    intent_evaluation_module = importlib.import_module("scripts.intent.evaluate_intent_policy")
