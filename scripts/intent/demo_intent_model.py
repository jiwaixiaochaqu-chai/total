...
使用方式：
    # 仅评估模型准确率
    python scripts/intent/demo_intent_model.py --eval-only

    # 对单个查询进行分类
    python scripts/intent/demo_intent_model.py "IT 设备采购流程怎么走"

    # 对追问类查询进行分类
    python scripts/intent/demo_intent_model.py "那第二种方案呢" --has-history

    # 输出治理报告
    python scripts/intent/demo_intent_model.py --output latest
"""
...
def main() -> None:
    ...
    eval_examples = load_intent_examples(args.eval_data)
    evaluation = service.evaluate(eval_examples)
    payload: dict[str, object] = {
        "model": service.model_version,
        "model_path": str(service.model_path),
        "labels": list(service.labels),
        "eval_dataset": str(Path(args.eval_data)),
