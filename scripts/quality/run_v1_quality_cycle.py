...
DEFAULT_RUNTIME_PARAM_RECOMMENDATION = "reports/runtime_params/v1_quality_cycle_recommendation.json"
...
class QualityCycleLock:
    ...
    调用顺序：main() -> with QualityCycleLock()。
    """
    ...
    def __init__(self, path: Path, stale_after_seconds: int = 12 * 60 * 60) -> None:
        ...
        参数：
            path: 锁文件路径。
            stale_after_seconds: 锁文件视为失效（可清理）的最小年龄，默认 12 小时。

        调用顺序：main() -> QualityCycleLock()。
        """
        self.path = path
    ...
    def __enter__(self) -> "QualityCycleLock":
        ...
        返回：
            当前锁对象（with 语句的 as 目标）。

        调用顺序：main() -> with QualityCycleLock() -> __enter__()。
        """
        self.path.parent.mkdir(parents=True, exist_ok=True)
        # 锁已存在时先判断年龄：未超过 stale_after_seconds（默认 12 小时）
        # 说明上一个周期可能仍在运行，直接拒绝
        if self.path.exists():
            age = time.time() - self.path.stat().st_mtime
            # 原因：正常周期远短于 12 小时，锁文件未过期说明上一个进程未正常退出
            # （仍在运行或被异常卡住），此时继续会让两个周期同时写同一批报告
            if age < self.stale_after_seconds:
                raise RuntimeError(f"质量周期正在运行或锁文件未过期：{self.path}")
            # 原因：超过 12 小时说明持有者已经死亡（崩溃或机器重启），
            # 锁文件失去保护意义，先删除以便下面原子重建
            self.path.unlink()
        # 原因：O_CREAT|O_EXCL 保证创建是原子的，并发竞态下只有一个进程能成功；
        # 失败方在 except FileExistsError 中被拒绝，避免"先删后建"的重叠执行窗口
    ...
    def __exit__(self, exc_type, exc, tb) -> None:
        """释放锁：无论正常退出还是异常都删除锁文件。

        参数：
            exc_type: 异常类型（无异常时为 None）。
            exc: 异常实例（无异常时为 None）。
            tb: traceback 对象（无异常时为 None）。

        返回：
            None（不吞没任何异常，调用方仍会收到原始异常）。

        调用顺序：with 语句退出 -> __exit__()。
        """
        # 原因：missing_ok=True 容忍"锁文件已被外部清理"的边界情况，
        # 避免清理动作自身再抛异常干扰正常退出流程
...
def _runner(use_docker: bool, *args: str) -> list[str]:
    ...
    参数：
        use_docker: 是否在 Compose api 容器内执行评测。
        args: 要执行的 python 脚本路径及其参数。

    返回：
        可直接交给 run_command_step 执行的命令参数列表。

    调用顺序：build_steps() -> _runner()。
    """
    if use_docker:
        # 原因：宿主机计划任务没有容器内的依赖与配置，通过 Compose run 复用
        # api 容器环境，保证无人值守评测与在线服务使用的环境一致
        return ["docker", "compose", "--env-file", ".env.compose", "run", "--rm", "api", "python", *args]
    # 原因：非 docker 模式直接用当前解释器（例如脚本自身已在容器内执行时，避免嵌套容器）
...
def build_steps(args: argparse.Namespace) -> list[tuple[str, list[str]]]:
    ...
    steps: list[tuple[str, list[str]]] = [
        ...
        (
            ...
            runner(
                "scripts/intent/evaluate_intent_policy.py",
        ...
        (
            ...
            runner(
                "scripts/intent/calibrate_thresholds.py",
        ...
        (
            ...
            runner(
                "scripts/quality/recommend_runtime_params.py",
                "--calibration-report", args.threshold_calibration_report,
                "--evaluation-report", args.evaluation_report,
                "--intent-policy-report", args.intent_policy_report,
                "--output", args.runtime_param_recommendation_report,
            ),
        ),
        (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--runtime-param-recommendation-report", default=DEFAULT_RUNTIME_PARAM_RECOMMENDATION)
        ...
        with QualityCycleLock(lock_path):
            ...
            payload = {
                ...
                "manual_boundary": [
                    ...
                    "人工批准意图模型训练、激活和回滚",
                    "人工复核运行时参数建议后，再修改配置并运行完整回归和性能门禁",
                ],
                "steps": [
                    {
                        "name": item.name,
